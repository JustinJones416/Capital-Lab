...
async function authLogin(){
  ...
  try {
    // Un intento que se cuelga por un bloqueador de anuncios o un
    // filtro de red suele ser un tropiezo puntual (la segunda
    // solicitud, ya con la conexión "calentada", frecuentemente sí
    // pasa) — antes de mostrar el error, se reintenta una vez en
    // silencio con un límite de tiempo más corto, en vez de obligar
    // a la persona a hacer manualmente lo que la app puede intentar
    // sola primero.
    let resultado;
    try {
      resultado = await conTiempoLimite(sb.auth.signInWithPassword({ email: correo, password }), 12);
    } catch(primerIntentoError){
      btn.innerHTML = '<span class="auth-spinner"></span> Reintentando…';
      resultado = await conTiempoLimite(sb.auth.signInWithPassword({ email: correo, password }), 12);
    }
...
async function authLoadProfileAndEnter(){
  // Antes estas dos llamadas no tenían ningún límite de tiempo — si
  // se colgaban aquí (después de un login ya exitoso), la persona se
  // quedaba con el botón girando para siempre, sin ningún mensaje de
  // error, ni siquiera el de "tardó demasiado".
  const { data: { user } } = await conTiempoLimite(sb.auth.getUser());
  if(!user){ authMsg('No se encontró la sesión. Intenta iniciar sesión de nuevo.'); return; }

  let { data: perfil, error } = await conTiempoLimite(sb.from('usuarios').select('*, sesiones_clase(nombre,codigo)').eq('auth_id', user.id).maybeSingle());
...
<p>Acceder, corregir, y solicitar la eliminación de sus datos, sujeto a obligaciones académicas de retención. Contacto: capitallabpty@gmail.com.</p>
...
const ALL_DERIVATIVES=[
  ...
  {id:'MSFT-CALL',name:'Opción Call Microsoft',ticker:'MSFT-C',sector:'Opciones',country:'EE.UU.',price:12.4,sigma:38.2,ret:11.4,type:'derivado',delta:0.36,gamma:0.07,theta:-0.06,vega:0.31,impliedVol:29.2,strike:11.16,intrinsic:7.44,profile:'Opción de compra (Call) sobre acciones de Microsoft. Otorga el derecho, no la obligación, de comprar el subyacente al precio de ejercicio mostrado arriba antes del vencimiento — se ejerce solo si conviene, y la pérdida máxima para el comprador es la prima pagada. Con una volatilidad implícita del 29.2%, el mercado espera movimientos más amplios en MSFT de lo habitual.',specs:{type:'Opciones',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'TSLA-PUT',name:'Opción Put Tesla',ticker:'TSLA-P',sector:'Opciones',country:'EE.UU.',price:18.6,sigma:52.4,ret:9.8,type:'derivado',delta:0.75,gamma:0.06,theta:-0.05,vega:0.3,impliedVol:42.4,strike:17.67,intrinsic:11.16,profile:'Opción de venta (Put) sobre acciones de Tesla. Otorga el derecho, no la obligación, de vender el subyacente al precio de ejercicio antes del vencimiento — se usa tanto para especular a la baja como para cubrir una posición larga en TSLA (comprar un put protege contra caídas, como un seguro). La volatilidad implícita de 42.4% es alta incluso para Tesla, una acción ya conocida por su volatilidad.',specs:{type:'Opciones',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'NVDA-CALL',name:'Opción Call NVIDIA',ticker:'NVDA-C',sector:'Opciones',country:'EE.UU.',price:24.8,sigma:48.7,ret:13.2,type:'derivado',delta:0.43,gamma:0.02,theta:-0.13,vega:0.38,impliedVol:46.7,strike:22.57,intrinsic:14.88,profile:'Opción de compra (Call) sobre acciones de NVIDIA. Otorga el derecho, no la obligación, de comprar el subyacente al precio de ejercicio antes del vencimiento. La volatilidad implícita de 46.7% refleja tanto la volatilidad histórica alta de NVDA como el interés del mercado por protegerse o especular sobre sus movimientos, típicamente ligados a resultados trimestrales y anuncios sobre IA.',specs:{type:'Opciones',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'SPX-PUT',name:'Opción Put S&P 500',ticker:'SPX-P',sector:'Opciones',country:'EE.UU.',price:42.5,sigma:28.4,ret:7.4,type:'derivado',delta:0.73,gamma:0.04,theta:-0.23,vega:0.28,impliedVol:36.4,strike:42.08,intrinsic:25.5,profile:'Opción de venta (Put) sobre el índice S&P 500. A diferencia de una opción sobre una sola acción, esta cubre el mercado accionario estadounidense en conjunto — es la herramienta clásica de cobertura de cartera: un inversionista con una cartera diversificada de acciones de EE.UU. compra puts sobre el SPX para protegerse de una caída generalizada, sin tener que vender sus posiciones individuales.',specs:{type:'Opciones',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'IRS-EUR',name:'Swap Tasa Interés EUR',ticker:'IRS-EUR',sector:'Swaps',country:'Eurozona',price:100.2,sigma:8.4,ret:4.2,type:'derivado',delta:0.5,gamma:0.03,theta:-0.1,vega:0.15,impliedVol:3.4,strike:110.22,intrinsic:60.12,profile:'Swap de tasa de interés en euros: dos partes intercambian pagos periódicos sobre el mismo monto nocional, una a tasa fija y la otra a tasa variable (referenciada al Euríbor), sin intercambiar el capital en ningún momento. Lo usa, por ejemplo, una empresa con deuda a tasa variable que quiere pasarse a pagos fijos y predecibles, sin tener que refinanciar la deuda original.',specs:{type:'Swaps',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'IRS-GBP',name:'Swap Tasa Interés GBP',ticker:'IRS-GBP',sector:'Swaps',country:'Reino Unido',price:99.8,sigma:9.1,ret:4.4,type:'derivado',delta:0.49,gamma:0.04,theta:-0.19,vega:0.24,impliedVol:13.1,strike:106.79,intrinsic:59.88,profile:'Swap de tasa de interés en libras esterlinas: dos partes intercambian pagos periódicos sobre el mismo monto nocional, una a tasa fija y la otra a tasa variable (referenciada a SONIA), sin intercambiar el capital. Permite a una empresa o inversionista cambiar su exposición a tasas de interés sin tocar la deuda o el activo original.',specs:{type:'Swaps',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'CDS-ARG',name:'CDS Argentina',ticker:'CDS-ARG',sector:'Crédito',country:'Argentina',price:62.4,sigma:42.8,ret:8.6,type:'derivado',delta:0.37,gamma:0.02,theta:-0.17,vega:0.12,impliedVol:44.8,strike:61.15,intrinsic:37.44,profile:'Credit Default Swap sobre deuda soberana de Argentina: funciona como un seguro contra el incumplimiento de pago (default) del país. El comprador paga una prima periódica (el "spread" del CDS) y, si Argentina no paga su deuda, recibe una compensación del vendedor. El spread se mueve con la percepción de riesgo país — cuanto más caro el CDS, más caro le resulta a Argentina endeudarse en los mercados internacionales.',specs:{type:'Crédito',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'CDS-BRA',name:'CDS Brasil',ticker:'CDS-BRA',sector:'Crédito',country:'Brasil',price:84.2,sigma:28.4,ret:6.8,type:'derivado',delta:0.44,gamma:0.07,theta:-0.14,vega:0.29,impliedVol:27.4,strike:76.62,intrinsic:50.52,profile:'Credit Default Swap sobre deuda soberana de Brasil: un seguro contra el incumplimiento de pago (default) del país. El comprador paga una prima periódica y, si Brasil no paga su deuda, recibe una compensación del vendedor. Su spread, más bajo que el de Argentina, refleja un riesgo país percibido como menor — pero sigue reaccionando a la política fiscal y monetaria brasileña.',specs:{type:'Crédito',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'CDS-TUR',name:'CDS Turquía',ticker:'CDS-TUR',sector:'Crédito',country:'Turquía',price:58.7,sigma:46.2,ret:9.1,type:'derivado',delta:0.37,gamma:0.06,theta:-0.17,vega:0.32,impliedVol:48.2,strike:56.94,intrinsic:35.22,profile:'Credit Default Swap sobre deuda soberana de Turquía: un seguro contra el incumplimiento de pago (default) del país. El comprador paga una prima periódica y, si Turquía no paga su deuda, recibe una compensación del vendedor. Turquía ha tenido episodios de alta inflación y presión sobre su moneda en años recientes, lo que explica que su CDS cotice con un spread más alto que el de otros mercados emergentes.',specs:{type:'Crédito',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'FWD-EUR',name:'Forward EUR/USD',ticker:'FWD-EUR',sector:'Forwards',country:'Eurozona',price:1.082,sigma:9.2,ret:2.4,type:'derivado',delta:0.69,gamma:0.02,theta:-0.09,vega:0.24,impliedVol:3.2,strike:1.06,intrinsic:0.65,profile:'Contrato forward de divisas: un acuerdo privado (no negociado en bolsa) para comprar o vender euros a cambio de dólares en una fecha futura, a un tipo de cambio fijado hoy. A diferencia de un futuro, un forward se negocia directamente entre dos partes (OTC) y sus términos se pueden ajustar a la medida — lo usan empresas con ingresos o pagos futuros en euros que quieren eliminar la incertidumbre del tipo de cambio.',specs:{type:'Forwards',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'FWD-JPY',name:'Forward USD/JPY',ticker:'FWD-JPY',sector:'Forwards',country:'Japón',price:151.4,sigma:10.4,ret:2.6,type:'derivado',delta:0.37,gamma:0.08,theta:-0.07,vega:0.12,impliedVol:2.4,strike:148.37,intrinsic:90.84,profile:'Contrato forward de divisas: un acuerdo privado para comprar o vender yenes a cambio de dólares en una fecha futura, a un tipo de cambio fijado hoy. Lo usan empresas japonesas con ventas en dólares (o empresas estadounidenses con costos en yenes) que quieren fijar el tipo de cambio de una transacción futura, sin esperar a la fecha real para saber a qué tipo de cambio van a operar.',specs:{type:'Forwards',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'FRA-USD',name:'FRA Dólar 3x6',ticker:'FRA-USD',sector:'Forwards',country:'EE.UU.',price:98.4,sigma:6.8,ret:3.4,type:'derivado',delta:0.5,gamma:0.07,theta:-0.1,vega:0.35,impliedVol:1.8,strike:89.54,intrinsic:59.04,profile:'Forward Rate Agreement (FRA): un acuerdo para fijar hoy la tasa de interés de un préstamo o depósito que empezará en el futuro. El "3×6" significa que la tasa aplica a un período de 3 meses que comienza dentro de 3 meses (es decir, cubre del mes 3 al mes 6 desde hoy). Se usa para eliminar la incertidumbre sobre a qué tasa se va a poder financiar o invertir dinero más adelante, sin comprometer el capital todavía.',specs:{type:'Forwards',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'CLO-EUR',name:'CLO Europeo AAA',ticker:'CLO-EUR',sector:'Estructurados',country:'Eurozona',price:99.1,sigma:12.4,ret:5.2,type:'derivado',delta:0.64,gamma:0.05,theta:-0.24,vega:0.39,impliedVol:21.4,strike:94.14,intrinsic:59.46,profile:'Collateralized Loan Obligation (CLO): un vehículo que agrupa cientos de préstamos corporativos (típicamente a empresas con calificación por debajo de grado de inversión) y los reempaqueta en tramos con distinto nivel de riesgo. El tramo AAA es el más seguro de todos — es el primero en cobrar y el último en absorber pérdidas si algunos de los préstamos subyacentes entran en incumplimiento, a cambio de un rendimiento más bajo que los tramos junior del mismo CLO.',specs:{type:'Estructurados',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'MBS-US',name:'MBS Estadounidense',ticker:'MBS-US',sector:'Estructurados',country:'EE.UU.',price:97.8,sigma:11.2,ret:4.8,type:'derivado',delta:0.59,gamma:0.06,theta:-0.09,vega:0.24,impliedVol:5.2,strike:92.91,intrinsic:58.68,profile:'Mortgage-Backed Security (MBS): un instrumento respaldado por un conjunto de hipotecas residenciales estadounidenses. Los pagos mensuales de capital e intereses de esas hipotecas se transfieren a los inversionistas del MBS. Su riesgo principal no es solo el incumplimiento de pago, sino el riesgo de prepago — si muchos propietarios refinancian sus hipotecas cuando bajan las tasas, el MBS se paga antes de lo esperado, reduciendo el rendimiento total para el inversionista.',specs:{type:'Estructurados',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'ABS-AUTO',name:'ABS Préstamos Auto',ticker:'ABS-AUT',sector:'Estructurados',country:'EE.UU.',price:98.6,sigma:9.8,ret:4.4,type:'derivado',delta:0.56,gamma:0.03,theta:-0.06,vega:0.11,impliedVol:0.8,strike:92.68,intrinsic:59.16,profile:'Asset-Backed Security (ABS) respaldado por un conjunto de préstamos para compra de automóviles. Funciona de forma similar a un MBS, pero con préstamos de auto en vez de hipotecas: los pagos mensuales de los deudores originales se transfieren a los inversionistas. Su plazo es más corto que el de un MBS típico, y el riesgo de prepago es menor porque refinanciar un préstamo de auto es menos común que refinanciar una hipoteca.',specs:{type:'Estructurados',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'VIX-CALL',name:'Opción Call VIX',ticker:'VIX-C',sector:'Opciones',country:'EE.UU.',price:8.4,sigma:68.4,ret:6.2,type:'derivado',delta:0.82,gamma:0.03,theta:-0.22,vega:0.37,impliedVol:75.4,strike:8.57,intrinsic:5.04,profile:'Opción de compra (Call) sobre el VIX, el índice que mide la volatilidad implícita esperada del S&P 500 (conocido como el "índice del miedo"). El VIX sube cuando el mercado espera turbulencia, así que este call gana valor en escenarios de pánico o incertidumbre — se usa como cobertura de cartera, ya que suele moverse en dirección contraria al mercado accionario. Su volatilidad implícita de 75.4% es de las más altas del catálogo: es volatilidad sobre la volatilidad misma.',specs:{type:'Opciones',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'GOLD-CALL',name:'Opción Call Oro',ticker:'GOLD-C',sector:'Opciones',country:'Global',price:32.6,sigma:24.8,ret:8.4,type:'derivado',delta:0.8,gamma:0.05,theta:-0.2,vega:0.35,impliedVol:29.8,strike:33.58,intrinsic:19.56,profile:'Opción de compra (Call) sobre el precio del oro. Otorga el derecho, no la obligación, de comprar oro al precio de ejercicio antes del vencimiento. El oro suele actuar como refugio de valor en momentos de inflación alta o inestabilidad económica, así que este instrumento se usa tanto para especular sobre su precio como para cubrir una cartera contra esos escenarios.',specs:{type:'Opciones',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'OIL-PUT',name:'Opción Put Petróleo',ticker:'OIL-P',sector:'Opciones',country:'Global',price:4.82,sigma:38.4,ret:5.8,type:'derivado',delta:0.4,gamma:0.05,theta:-0.2,vega:0.35,impliedVol:43.4,strike:4.96,intrinsic:2.89,profile:'Opción de venta (Put) sobre el precio del petróleo crudo. Otorga el derecho, no la obligación, de vender al precio de ejercicio antes del vencimiento — la usan tanto especuladores que anticipan una caída del precio como productores petroleros que quieren asegurar un precio mínimo de venta para su producción futura, sin comprometerse a vender a ese precio si el mercado sube.',specs:{type:'Opciones',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  ...
  {id:'VARSWAP',name:'Swap de Varianza',ticker:'VAR-SW',sector:'Swaps',country:'Global',price:98.2,sigma:32.8,ret:7.4,type:'derivado',delta:0.61,gamma:0.06,theta:-0.21,vega:0.16,impliedVol:38.8,strike:91.33,intrinsic:58.92,profile:'Instrumento que permite apostar directamente sobre la volatilidad futura de un activo, en vez de sobre su precio. Al vencimiento, una parte paga a la otra la diferencia entre la volatilidad realizada (la que efectivamente ocurrió) y una volatilidad fija pactada de antemano — se usa para cubrir carteras de opciones cuyo valor depende fuertemente de la volatilidad, o para especular sobre si el mercado va a moverse más o menos de lo que el mercado mismo espera.',specs:{type:'Swaps',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'CAP-USD',name:'Cap Tasa USD',ticker:'CAP-USD',sector:'Opciones',country:'EE.UU.',price:2.84,sigma:18.4,ret:4.2,type:'derivado',delta:0.67,gamma:0.08,theta:-0.07,vega:0.32,impliedVol:10.4,strike:2.58,intrinsic:1.7,profile:'Cap de tasa de interés en dólares: un seguro contra el alza de tasas. Quien lo compra (típicamente alguien con deuda a tasa variable) recibe un pago cada vez que la tasa de referencia sube por encima del nivel pactado (el "strike" del cap), limitando así el costo máximo de su financiamiento — sin renunciar al beneficio si las tasas bajan, a diferencia de un swap que fija la tasa por completo.',specs:{type:'Opciones',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'FLOOR-EUR',name:'Floor Tasa EUR',ticker:'FLR-EUR',sector:'Opciones',country:'Eurozona',price:2.42,sigma:16.8,ret:3.8,type:'derivado',delta:0.69,gamma:0.06,theta:-0.09,vega:0.24,impliedVol:10.8,strike:2.23,intrinsic:1.45,profile:'Floor de tasa de interés en euros: el espejo del cap. Quien lo compra (típicamente un inversionista con activos a tasa variable, como un bono) recibe un pago cada vez que la tasa de referencia cae por debajo del nivel pactado, asegurando un rendimiento mínimo sobre su inversión sin importar qué tan bajo lleguen las tasas del mercado.',specs:{type:'Opciones',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'BARRIER-SPX',name:'Opción Barrera S&P',ticker:'BAR-SPX',sector:'Opciones',country:'EE.UU.',price:18.4,sigma:34.2,ret:8.8,type:'derivado',delta:0.61,gamma:0.02,theta:-0.21,vega:0.36,impliedVol:40.2,strike:19.69,intrinsic:11.04,profile:'Opción exótica sobre el S&P 500 cuyo derecho a ejercer depende de que el índice toque (o no toque) un nivel de precio específico durante la vida del contrato, además del precio de ejercicio habitual. Al agregar esa condición extra, suele costar menos prima que una opción tradicional (vanilla) equivalente — a cambio de ese riesgo adicional de que la barrera se active o desactive el contrato antes del vencimiento.',specs:{type:'Opciones',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}},
  {id:'CONVERT-AAPL',name:'Bono Convertible Apple',ticker:'CONV-AAPL',sector:'Estructurados',country:'EE.UU.',price:104.2,sigma:18.6,ret:7.2,type:'derivado',delta:0.83,gamma:0.08,theta:-0.23,vega:0.18,impliedVol:26.6,strike:108.37,intrinsic:62.52,profile:'Bono corporativo de Apple que combina renta fija con una opción incorporada: el tenedor puede convertirlo en un número fijo de acciones de AAPL antes del vencimiento, en vez de recibir el pago en efectivo. Esto le da al inversionista lo mejor de ambos mundos hasta cierto punto — cobra un cupón mientras el bono no se convierte, y participa de la subida de AAPL si decide convertir — a cambio de un cupón más bajo que un bono corporativo tradicional sin esa opción.',specs:{type:'Estructurados',underlying:'Diversos',notional:'$100,000',maturity:'3-12 meses',market:'OTC'}}
];
